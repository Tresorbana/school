...
export class AuthService {
    static async login(email: string, pass: string) {
        try {
            ...
            const roleCodes = roles.map((r) => NameToCodeMap[r.name]).filter(code => code !== undefined);
    ...
    static async signup(firstName: string, lastName: string, email: string, pass: string) {
        try {
            ...
            return { status: 'error', message: `Signup failed: ${(error as Error).message}` };
        }
    }
