...
export class StudentService {
    static async getAllStudents(params: {
        ...
        if (filters.intake_id) where.intake_id = filters.intake_id;
    ...
    static async assignToIntake(studentIds: string[], intakeId: string) {
    ...
    static async assignToClass(studentIds: string[], classId: string) {
        const result = await prisma.student.updateMany({
            where: { student_id: { in: studentIds } },
    ...
    static async bulkCreate(students: any[]) {
