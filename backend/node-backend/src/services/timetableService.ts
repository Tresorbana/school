...
export class TimetableService {
    static async createTimetable(academicYear: string, classId: string, term: number) {
    ...
    static async assignSlot(rosterId: string, assignmentId: string) {
        const roster = await prisma.timetableRoster.findUnique({
            where: { roster_id: rosterId },
            include: { timetable: true }
        });

        if (!roster) throw new Error('Slot not found');
    ...
    static async activateTimetable(id: string) {
    ...
    static async getTeacherTodaySchedule(teacherId: string, date: Date = new Date()) {
        const today = date.toLocaleDateString('en-US', { weekday: 'long' }) as DayOfWeek;
