...
function AddCourseModal({ isOpen, onClose, onRefresh }: AddCourseModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.courseName.trim() || !formData.yearLevel) {
    ...
    try {
      ...
      if (response.success) {
        ...
        setFormData({ courseName: "", yearLevel: "" });
        onRefresh?.(); // Refresh the parent data
        onClose();
      }
    } catch (error: any) {
      addToast({ message: 'Failed to create course: ' + error.message, type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
