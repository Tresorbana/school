...
import { intakeService, studentService } from "../../../services/academicService";
...
function AddIntakeModal({ isOpen, onClose, onSuccess }: AddIntakeModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate intake name format on frontend
    ...
    try {
      ...
      // Calculate start_year and end_year from intake name
      const startYear = firstYear;
      const endYear = firstYear + 3;
      
      const response = await intakeService.createIntake({
      ...
      if (response.success) {
        const intakeId = response.data?.intake_id;
        if (createdStudentIds.length > 0 && intakeId) {
          const assignResponse = await studentService.bulkAssignToIntake(createdStudentIds, intakeId);
        ...
        addToast({ message: 'Intake created successfully', type: 'success' });
        setFormData({
          intake_name: "",
          current_year_level: 1
        });
      ...
      addToast({ message: 'Failed to create intake: ' + error.message, type: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <ModalWrapper isOpen={isOpen} onClose={onClose} className="w-full max-w-md max-h-[90vh]">
      <div className="bg-white rounded-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center px-4 py-2 border-b sticky top-0 bg-white">
          <h2 className="text-xs font-semibold">Add new intake</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4">
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Format: YYYY-YYYY (academic year, e.g., 2024-2025)
            </p>
          </div>
          ...
          {isImportOpen && (
            <div className="mt-3">
              ...
              <div
                ...
                onDrop={handleDrop}
                onClick={() => document.getElementById('add-intake-csv-input')?.click()}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    document.getElementById('add-intake-csv-input')?.click();
                  }
                }}
              >
                <input
                  id="add-intake-csv-input"
                  type="file"
                  accept=".csv"
                  className="hidden"
